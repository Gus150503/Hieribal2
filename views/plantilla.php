...
<head>
  ...
  (function () {
    try {
      ...
      apply(); if (pref==='auto' && window.matchMedia){ var mq=matchMedia('(prefers-color-scheme: dark)'); (mq.addEventListener||mq.addListener).call(mq,'change',apply); }
    } catch(e){}
  })();
  </script>
...
<?php if ($isAdmin): ?>
  ...
  <div class="admin-wrap">
    ...
    <main class="content p-3">
      ...
      <button id="sidebarToggle" class="btn btn-light btn-sm mb-3" type="button" aria-expanded="false" aria-label="Alternar menú">
        <i class="bi bi-list"></i>
      </button>

      <?= $contenido ?? '' ?>  <!-- aquí se inyecta dashboard.php -->
    </main>
  </div>
  ...
  <?php if ($isAdmin): ?>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      addEventListener('resize', () => {
        if (isDesktop()) closeM(); else document.body.classList.remove('sidebar-collapsed');
      });
    });
    </script>
  <?php endif; ?>
